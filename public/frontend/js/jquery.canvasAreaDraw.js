...
(function ($) {
    ...
    var init = function (index, input, options) {
        ...
        if (v.length) {
            points = v.split(',').map(function (point) {//get number from input field in base 10.
                console.log(points);
        ...
        $reset = $('<button type="button" class="btn btn-xs"><i class="icon-trash"></i>Reset</button>');
        $addNew = $('<button type="button" class="btn btn-xs"><i class="icon-pencil"></i>Add New</button>');
        $clearLast = $('<button type="button" class="btn btn-xs"><i class="icon-undo"></i>Clear Last</button>');
        $save = $('<button type="button" class="btn btn-xs btn-success"><i class="icon-save"></i>Save</button>');
        $canvas = $('<canvas>');
        ctx = $canvas[0].getContext('2d');

        image = new Image();
        resize = function () {
            $canvas.attr('height', 450).attr('width', 900);
            draw();
        };
        $(image).load(resize);
        ...
        $canvas.css({backgroundPosition: 'center center'});
        $canvas.css({backgroundSize: 'cover'});

        $(input).after('<br>', $canvas, '<br>', $reset,$addNew,$clearLast,$save);
        ...
        save = function(){
            //$("#mycanvas").val(multiplePoints);
            /*foreach(multiplePoints as multiplePoint){
                console.log(multiplepoint);

            }*/
            /*multiplePoints.each(function() {
              alert('wow');
            });*/
            for(var i=0; i<multiplePoints.length; i++){
                //alert('wow'+i);
                $('#myform').append('<input type="hidden" name="coordinate[]" value="'+multiplePoints[i]+'" >');
            }
            //console.log(multiplePoints);
            $('#myform').submit();
            return false;
        ...
        $(document).find($clearLast).click(clearLast);
